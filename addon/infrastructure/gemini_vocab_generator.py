...
from domain.vocab_generator import VocabGenerator
...
class GeminiVocabGenerator(VocabGenerator):
    ...
    def generate_vocab(self, user_qa: UserQA, target_language) -> VocabList:
        ...
        {"\n".join(f"Q{i}: {q}" for i, q in enumerate(user_qa.questions, start=1))}
        ...
        {"\n".join(f"A{i}: {a}" for i, a in enumerate(user_qa.answers, start=1))}
        """
